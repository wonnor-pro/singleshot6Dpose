...
def valid(epoch, niter):
    ...
    logging("   Validating...")
    logging("   Number of validate samples: %d" % len(valid_loader.dataset))
    notpredicted = 0
    # Iterate through test examples
...
if __name__ == "__main__":
    ...
    valid_loader = torch.utils.data.DataLoader(dataset.listDataset(validatelist,
                                                                  shape=(test_width, test_height),
                                                                  shuffle=False,
    ...
    if evaluate:
        ...
        for epoch in range(init_epoch, max_epochs): 
            ...
            if (epoch % 10 == 0) and (epoch is not 0): 
                test(epoch, niter, save_threshold=5)
